...
//		}
	...
	private void findPathForEachFlow() {
		for (Job j : jobs) {
			for (Task t : j.tasks) {
				if (t.taskType != TaskType.REDUCER) {
					continue;
				...
				ReduceTask rt = (ReduceTask) t;
				for (Flow flow : rt.flows) {
					int srcRack = flow.mapper.taskID;
					int desRack = rt.taskID;
					ODPair o = new ODPair(srcRack, desRack);
				}
			}
		}
	}
	 ...
	 *
	 * @param job   coflow to calculate for
	 * @param sFree free bps in uplinks
	 * @param rFree free bps in downlinks
	 * @return
	 */
	 ...
	 * @param sendUsed       initialized to zeros.
	 * @param recvUsed       initialized to zeros.
	 * @return
	 */
