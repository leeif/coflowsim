...
public class Job implements Comparable<Job> {
	 ...
	 * @param jobID   a unique ID of the job.
	 */
	public Job(String jobName, int jobID, Constants.ROUTING_ALGO routingAlgo) {
	 ...
	 *
	 * @param task {@link coflowsim.datastructures.Task} to add.
	 */
	 ...
	 *
	 * @param task {@link coflowsim.datastructures.Task} under consideration.
	 */
	...
	public void arrangeTasks(int numRacks, int machinesPerRack, double deadlineMult) {
		...
		if (mapperInRacks == null) {
			mapperInRacks = new Task[numRacks];
		}
	...
	private void coalesceMappers(int numRacks) {
		...
		for (int i = 0; i < numRacks; i++) {
			if (numMappersInRacks[i] > 0) {
				...
				MapTask iThMt;
				if (routingAlgo == Constants.ROUTING_ALGO.SA) {
					iThMt = new MapTask("JOB-" + jobID + "-MAP-" + i, i, this, actualStartTime,
									Constants.VALUE_UNKNOWN, new Machine(mapperInRacks[i].assignedMachine.machineID));
				} else {
					iThMt = new MapTask("JOB-" + jobID + "-MAP-" + i, i, this, actualStartTime,
									Constants.VALUE_UNKNOWN, new Machine(i));
				}
	 ...
	 *
	 * @param rackID     index of the rack
	 * @param decreaseBy bytes transferred (i.e., to delete)
	 */
